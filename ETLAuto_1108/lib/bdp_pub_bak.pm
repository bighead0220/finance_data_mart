...
sub decDate {
   ...
   if($count > 0){
      for (; $count > 0 ; $count --) {
         ...
         if(index(',0131,0331,0430,0531,0630,0731,0831,0930,1031,1130,',"$md") >= 0){
            $date = $y.sprintf("%02d",$m + 1)."01";
         }elsif($md eq ',1231,'){
            $date = sprintf("%04d",$y + 1)."0101";
         }else{
            if(($md eq ',0228,' && !($y % 400 ==0 || ($y % 4 == 0 && $y % 100 != 0))) ||
               ($md eq ',0229,' &&  ($y % 400 ==0 || ($y % 4 == 0 && $y % 100 != 0)))){
               $date = $y."0301";
            }else{
               $date += 1;
            }
      ...
      for (; $count > 0 ; $count --) {
         ...
         if ($d > 1) {
            ...
            if($m == 1){
               ...
               $date = $y . sprintf("%02d",$m - 1);
               if(index(',02,04,06,08,09,11,',",$m,") >= 0){
                  $date = $date . "31";
               }else{
                  $date = $date . "30";
               }
