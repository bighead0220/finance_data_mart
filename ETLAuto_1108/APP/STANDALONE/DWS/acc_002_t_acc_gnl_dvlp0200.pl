...
--Step0:
DELETE FROM dw_sdata.ACC_002_T_ACC_GNL_DVLP WHERE start_dt>=DATE('${TX_DATE_YYYYMMDD}');
UPDATE dw_sdata.ACC_002_T_ACC_GNL_DVLP SET end_dt=DATE('2100-12-31') WHERE end_dt>=DATE('${TX_DATE_YYYYMMDD}') AND end_dt<>DATE('2100-12-31');

--Step1:
CREATE LOCAL TEMPORARY TABLE  T_9 ON COMMIT PRESERVE ROWS AS SELECT * FROM dw_sdata.ACC_002_T_ACC_GNL_DVLP WHERE 1=0;
...
FROM 
 ...
 FROM  dw_tdata.ACC_002_T_ACC_GNL_DVLP_${TX_DATE_YYYYMMDD}) N
LEFT JOIN
 (SELECT 
  ITM_NO ,
  ACC_SET ,
  ...
  SND_CRT_NUM ,
  FLAG 
 FROM dw_sdata.ACC_002_T_ACC_GNL_DVLP 
 WHERE END_DT = DATE('2100-12-31') ) T
ON N.ITM_NO = T.ITM_NO AND N.ACC_SET = T.ACC_SET
WHERE
(T.ITM_NO IS NULL AND T.ACC_SET IS NULL)
...
--Step3:
UPDATE dw_sdata.ACC_002_T_ACC_GNL_DVLP P 
SET End_Dt=DATE('${TX_DATE_YYYYMMDD}')
FROM T_9
WHERE P.End_Dt=DATE('2100-12-31')
AND P.ITM_NO=T_9.ITM_NO
AND P.ACC_SET=T_9.ACC_SET
;

--Step4:
INSERT  INTO dw_sdata.ACC_002_T_ACC_GNL_DVLP SELECT * FROM T_9;
