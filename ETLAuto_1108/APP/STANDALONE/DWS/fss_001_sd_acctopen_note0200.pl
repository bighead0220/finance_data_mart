...
--Step0:
DELETE FROM dw_sdata.FSS_001_SD_ACCTOPEN_NOTE WHERE start_dt>=DATE('${TX_DATE_YYYYMMDD}');
UPDATE dw_sdata.FSS_001_SD_ACCTOPEN_NOTE SET end_dt=DATE('2100-12-31') WHERE end_dt>=DATE('${TX_DATE_YYYYMMDD}') AND end_dt<>DATE('2100-12-31');

--Step1:
CREATE LOCAL TEMPORARY TABLE  T_197 ON COMMIT PRESERVE ROWS AS SELECT * FROM dw_sdata.FSS_001_SD_ACCTOPEN_NOTE WHERE 1=0;
...
FROM 
 (SELECT
  ...
  COALESCE(OPERFLAG, '' ) AS OPERFLAG ,
  COALESCE(OPERCODE, '' ) AS OPERCODE ,
  COALESCE(SENDFLAG, '' ) AS SENDFLAG ,
  COALESCE(AFFIRMFLAG, '' ) AS AFFIRMFLAG ,
  COALESCE(CHNLNO, '' ) AS CHNLNO ,
  COALESCE(ENCRYPYNO, '' ) AS ENCRYPYNO 
 FROM  dw_tdata.FSS_001_SD_ACCTOPEN_NOTE_${TX_DATE_YYYYMMDD}) N
LEFT JOIN
 (SELECT 
  CUSTOMERID ,
  SAVINGBONDACCT ,
  ...
  CHNLNO ,
  ENCRYPYNO 
 FROM dw_sdata.FSS_001_SD_ACCTOPEN_NOTE 
 WHERE END_DT = DATE('2100-12-31') ) T
ON N.CUSTOMERID = T.CUSTOMERID
WHERE
(T.CUSTOMERID IS NULL)
 OR N.SAVINGBONDACCT<>T.SAVINGBONDACCT
 ...
 OR N.OPERFLAG<>T.OPERFLAG
 OR N.OPERCODE<>T.OPERCODE
 OR N.SENDFLAG<>T.SENDFLAG
 OR N.AFFIRMFLAG<>T.AFFIRMFLAG
 OR N.CHNLNO<>T.CHNLNO
 OR N.ENCRYPYNO<>T.ENCRYPYNO
;

--Step3:
UPDATE dw_sdata.FSS_001_SD_ACCTOPEN_NOTE P 
SET End_Dt=DATE('${TX_DATE_YYYYMMDD}')
FROM T_197
WHERE P.End_Dt=DATE('2100-12-31')
AND P.CUSTOMERID=T_197.CUSTOMERID
;

--Step4:
INSERT  INTO dw_sdata.FSS_001_SD_ACCTOPEN_NOTE SELECT * FROM T_197;
