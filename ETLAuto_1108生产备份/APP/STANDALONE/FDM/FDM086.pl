...
#---------------
#Author:Liuxz
#Function:将dw_sdata.cos_000_sectype where transtype='SE' and owner in ('2312','2310')的叶子节点（不包括中间节点）全部递归出来，并插入F_FDM.CD_CD_TABLE作为FDM086代码类
#---------------
# ------------ program section ------------
use strict;
use bdp_pub;
...
$all = &run_vsql_comm("select thekey,ext_name,owner,thelevel from dw_sdata.cos_000_sectype where transtype='SE' and start_dt<='${DB_WORK_DATE}'::date and end_dt>'${DB_WORK_DATE}'::date",$connect_string);
