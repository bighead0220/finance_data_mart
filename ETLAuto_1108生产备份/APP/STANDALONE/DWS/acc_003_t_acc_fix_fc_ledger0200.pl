...
--Step0:
DELETE FROM dw_sdata.ACC_003_T_ACC_FIX_FC_LEDGER WHERE start_dt>=DATE('${TX_DATE_YYYYMMDD}');
UPDATE dw_sdata.ACC_003_T_ACC_FIX_FC_LEDGER SET end_dt=DATE('2100-12-31') WHERE end_dt>=DATE('${TX_DATE_YYYYMMDD}') AND end_dt<>DATE('2100-12-31');

--Step1:
CREATE LOCAL TEMPORARY TABLE  T_19 ON COMMIT PRESERVE ROWS AS SELECT * FROM dw_sdata.ACC_003_T_ACC_FIX_FC_LEDGER WHERE 1=0;
...
FROM 
 ...
 FROM  dw_tdata.ACC_003_T_ACC_FIX_FC_LEDGER_${TX_DATE_YYYYMMDD}) N
LEFT JOIN
 (SELECT 
  ACC ,
  SYS_CODE ,
  ...
  EXPAND_4 ,
  EXPAND_5 
 FROM dw_sdata.ACC_003_T_ACC_FIX_FC_LEDGER 
...
--Step3:
UPDATE dw_sdata.ACC_003_T_ACC_FIX_FC_LEDGER P 
SET End_Dt=DATE('${TX_DATE_YYYYMMDD}')
FROM T_19
WHERE P.End_Dt=DATE('2100-12-31')
AND P.ACC=T_19.ACC
AND P.SYS_CODE=T_19.SYS_CODE
;

--Step4:
INSERT  INTO dw_sdata.ACC_003_T_ACC_FIX_FC_LEDGER SELECT * FROM T_19;
