...
--Step0:
DELETE FROM dw_sdata.ACC_003_T_ACC_CDM_COM_LEDGER WHERE start_dt>=DATE('${TX_DATE_YYYYMMDD}');
UPDATE dw_sdata.ACC_003_T_ACC_CDM_COM_LEDGER SET end_dt=DATE('2100-12-31') WHERE end_dt>=DATE('${TX_DATE_YYYYMMDD}') AND end_dt<>DATE('2100-12-31');

--Step1:
CREATE LOCAL TEMPORARY TABLE  T_15 ON COMMIT PRESERVE ROWS AS SELECT * FROM dw_sdata.ACC_003_T_ACC_CDM_COM_LEDGER WHERE 1=0;
...
FROM 
 (SELECT
  ...
  COALESCE(LONG_BAL, 0 ) AS LONG_BAL ,
  ...
  COALESCE(ECON_SEC_CODE, '' ) AS ECON_SEC_CODE ,
 ...
 FROM  dw_tdata.ACC_003_T_ACC_CDM_COM_LEDGER_${TX_DATE_YYYYMMDD}) N
LEFT JOIN
 (SELECT 
  ACC ,
  SYS_CODE ,
  ...
  EXPAND_4 ,
  EXPAND_5 
 FROM dw_sdata.ACC_003_T_ACC_CDM_COM_LEDGER 
...
(T.ACC IS NULL AND T.SYS_CODE IS NULL)
 ...
 OR N.ECON_SEC_CODE<>T.ECON_SEC_CODE
...
--Step3:
UPDATE dw_sdata.ACC_003_T_ACC_CDM_COM_LEDGER P 
SET End_Dt=DATE('${TX_DATE_YYYYMMDD}')
FROM T_15
WHERE P.End_Dt=DATE('2100-12-31')
AND P.ACC=T_15.ACC
AND P.SYS_CODE=T_15.SYS_CODE
;

--Step4:
INSERT  INTO dw_sdata.ACC_003_T_ACC_CDM_COM_LEDGER SELECT * FROM T_15;
